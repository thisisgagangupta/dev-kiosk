...
function toSortedUniqueHHMM(list: string[]): string[] {
  // normalize + keep only strict HH:mm
  const norm = list
    .map((s) => normalizeHHMM(s))
    .filter((s) => HHMM_RE.test(s));
  const uniq = Array.from(new Set(norm));
  // sort on a COPY (eslint/immutability-friendly)
  return [...uniq].sort(compareHHMM);
}
...
const DOCTORS: Doctor[] = [
  {
    ...
    clinicName: "MedMitra Downtown Clinic",
    rating: 4.8,
  ...
  },
  {
    ...
    specialty: "General Medicine",
    clinicName: "MedMitra Central Clinic",
    rating: 4.6,
    qualifications: "MBBS, MD",
  },
];
...
export default function WalkinSlotPage() {
  ...
  const groupSize = Math.max(
    1,
    Number(sessionStorage.getItem("kioskGroupSize") || "1")
  );
  ...
  const allSlots = useMemo(
    () => generateQuarterHourSlots("08:00", "20:00"),
    []
  );
  const availableSlots = useMemo(() => {
    if (!selectedDate) return [];
  ...
  useEffect(() => {
    if (!patientId) {
      toast({
        variant: "destructive",
        title: t("common.sessionExpiredTitle", "Session Expired"),
        description: t(
    ...
    const fetchAvailability = async () => {
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(
            data.detail ||
              `Failed to load availability (${res.status})`
          );
        if (!aborted) {
          // normalize to strict HH:mm once here
          const normalized = (data.booked || []).map((s: string) =>
            normalizeHHMM(s)
          );
          setBooked(toSortedUniqueHHMM(data.booked || []));
          // remove any user-selected slot that just became booked
          setSelectedSlots((prev) =>
            prev.filter((s) => !normalized.includes(s))
          );
        }
      } catch (e: any) {
        if (!aborted) {
          setError(
            e?.message ||
              t(
                "walkinSlot.errors.loadAvailability",
  ...
  function toggleSlot(slot: string) {
    ...
    if (selectedSlots.length >= groupSize) {
      const titleTemplate = t(
        "walkinSlot.errors.needNSlotsTitle",
        `You can select up to __N__ slot${
          groupSize > 1 ? "s" : ""
        }`
      );
      const descTemplate = t(
        "walkinSlot.errors.needNSlotsDesc",
        "Deselect a time to pick another."
      );
      toast({
        variant: "destructive",
        title: titleTemplate.replace("__N__", String(groupSize)),
  ...
  async function handleBook() {
    if (!patientId || !doctor || !selectedDate || selectedSlots.length === 0)
      return;
    if (selectedSlots.length !== groupSize) {
      const titleTemplate = t(
        "walkinSlot.errors.needNSlotsTitle",
        `Please select __N__ slot${
          groupSize > 1 ? "s" : ""
        }`
      );
      const descTemplate = t(
        "walkinSlot.errors.needNSlotsDesc",
        "Currently selected: __CURR__"
      );
      toast({
        variant: "destructive",
        title: titleTemplate.replace("__N__", String(groupSize)),
        description: descTemplate.replace(
          "__CURR__",
    ...
    try {
      ...
      if (selectedSlots.length === 1) {
        ...
        if (res.status === 409) {
          ...
          toast({
            variant: "destructive",
            title: t(
              "walkinSlot.errors.slotUnavailableTitle",
              "Slot Unavailable"
            ),
            description:
              data.detail ||
              t(
                "walkinSlot.errors.slotUnavailableDesc",
        ...
        const res = await fetch(
          `${API_BASE}/api/appointments/book-batch`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
        ...
        if (res.status === 409) {
          ...
          toast({
            variant: "destructive",
            title: t(
              "walkinSlot.errors.someSlotsTakenTitle",
              "Some slots just got taken"
            ),
            description: bad
              ? t(
                  "walkinSlot.errors.someSlotsTakenDesc",
                  `Conflicts: ${bad}`
                )
              : t(
                  "walkinSlot.errors.someSlotsTakenFallbackDesc",
        ...
        if (!res.ok)
          throw new Error(
            data.detail || `Batch booking failed (${res.status})`
          );
        const first = Array.isArray(data.appointments)
          ? data.appointments[0]
          : null;
        if (first?.appointmentId) {
          sessionStorage.setItem(
      ...
      const successTitle = t(
        "walkinSlot.successTitle",
        "Appointment(s) Booked"
      );
      const desc =
        selectedSlots.length > 1
          ? t(
              "walkinSlot.successDescMultiple",
              `Booked ${selectedSlots.length} slot(s).`
            )
          : t(
              "walkinSlot.successDescSingle",
              "Booking confirmed."
            );
      toast({
        title: successTitle,
      ...
      setError(
        e?.message ||
          t(
            "walkinSlot.errors.bookingFailed",
  ...
  async function refreshAfterRace(dateISO: string) {
    try {
      ...
      const r2 = await fetch(url.toString());
      const d2 = await r2.json().catch(() => ({}));
      const normalized = (d2.booked || []).map((s: string) =>
        normalizeHHMM(s)
      );
      setBooked(normalized);
      setSelectedSlots((prev) =>
  ...
  const subtitleSingle = t(
    "walkinSlot.subtitleSingle",
    "Select your preferred doctor, date, and time slot"
  );
  const subtitleGroupTemplate = t(
    "walkinSlot.subtitleGroup",
    "Select __N__ separate time slot(s) for your group"
  );
  const subtitleGroup = subtitleGroupTemplate.replace(
    "__N__",
    String(groupSize)
  );

  // Selected section label template
  const selectedLabelTemplate = t(
    "walkinSlot.selectedLabel",
    "Selected (__SEL__/__TOTAL__)"
  );
  const selectedLabel = selectedLabelTemplate
    .replace("__SEL__", String(selectedSlots.length))
    .replace("__TOTAL__", String(groupSize));

  return (
    <KioskLayout
      title={t("walkinSlot.title", "Book Walk-in Appointment")}
    >
      <div className="max-w-7xl mx-auto px-4">
        {/* Header */}
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-3">
            {t(
              "walkinSlot.title",
              "Book Your Walk-in Appointment"
            )}
          </h1>
          <p className="text-lg text-muted-foreground">
            {groupSize > 1 ? subtitleGroup : subtitleSingle}
          </p>
        </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-destructive">
                    {t("walkinSlot.bookingErrorTitle", "Booking Error")}
                  </p>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="shadow-md">
              <CardHeader className="space-y-1 pb-4">
                ...
                <CardDescription>
                  {t(
                    "walkinSlot.doctorSubtitle",
                    "Select from available walk-in doctors"
                  )}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium">
                        {t(
                          "walkinSlot.groupBookingTitle",
                          "Group Booking"
                        )}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {t(
                          "walkinSlot.groupBookingDesc",
                          "Select __N__ separate time slots for your group."
                        ).replace("__N__", String(groupSize))}
                      </p>
                    </div>
                    <Badge
                      variant="secondary"
            ...
            <Card className="shadow-md">
              <CardHeader className="space-y-1 pb-4">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="flex items-center gap-2">
                    <CalendarIcon className="h-5 w-5" />
                    {t("walkinSlot.selectDate", "Select Date")}
                  </CardTitle>
                </div>
                <CardDescription>
                  {t(
                    "walkinSlot.selectDateSubtitle",
              ...
              <CardHeader className="space-y-1 pb-4">
                <div className="flex items-center justify-between">
                  ...
                  {earliest && (
                    ...
                    >
                      <Clock className="h-3 w-3" />
                      {t("walkinSlot.nextLabel", "Next")}: {earliest}
                    </Badge>
                  )}
                </div>
                <CardDescription>
                  {selectedDate
                    ? t(
                        "walkinSlot.availableSlotsFor",
                        "Available slots for __DATE__"
                      ).replace(
                        "__DATE__",
                        dateToLocalYYYYMMDD(selectedDate)
                      )
                    : t(
                        "walkinSlot.selectDateFirst",
                        "Select a date first"
                      )}
                </CardDescription>
              </CardHeader>
                ...
                {loading ? (
                  <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                    ...
                    <p className="text-lg font-medium">
                      {t(
                        "walkinSlot.loadingSlotsTitle",
                        "Loading available slots..."
                      )}
                    </p>
                    <p className="text-sm">
                      {t("walkinSlot.loadingSlotsSubtitle", "Please wait")}
                    </p>
                  </div>
                ) : availableSlots.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                    ...
                    <p className="text-lg font-medium">
                      {t(
                        "walkinSlot.noSlotsTitle",
                        "No slots available"
                      )}
                    </p>
                    <p className="text-sm">
                      {t(
                        "walkinSlot.noSlotsSubtitle",
                    ...
                    {selectedSlots.length > 0 && (
                      <div className="p-4 rounded-lg bg-muted/50 border">
                        <div className="flex items-start gap-3">
                          ...
                          >
                            <XCircle className="h-4 w-4 mr-1" />
                            {t("walkinSlot.clearSelection", "Clear")}
                          </Button>
                        </div>
                      </div>
                    )}
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-3 pb-3 border-b">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-muted-foreground mb-1">
                          {t("walkinSlot.summaryDate", "Date")}
                        </p>
                        <p className="font-medium text-sm">
                          {selectedDate
                            ? dateToLocalYYYYMMDD(selectedDate)
                            : t(
                                "walkinSlot.dateNotSelected",
                                "Not selected"
                              )}
                        </p>
                      </div>
                    ...
                    <div className="flex items-start gap-3 pb-3 border-b">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-muted-foreground mb-1">
                          {t(
                            "walkinSlot.summarySelectedTimes",
                            "Selected Time"
                          )}
                      ...
                      {booking ? (
                        ...
                        <>
                          ...
                          {groupSize > 1
                            ...
                            : t(
                                "walkinSlot.bookingButtonSingle",
                                "Confirm & Continue"
                              )}
                        </>
                      )}
                  ...
                  {selectedSlots.length !== groupSize && (
                    <p className="text-xs text-center text-muted-foreground">
                      {t(
                        "walkinSlot.selectSlotsHint",
                        "Please select __N__ slot(s) to continue"
                      ).replace("__N__", String(groupSize))}
                    </p>
                  )}
                </CardContent>
              ...
              <Card className="bg-muted/50">
                <CardContent className="pt-6">
                  <div className="text-center space-y-2">
                    <p className="text-sm font-medium">
                      {t(
                        "walkinSlot.assistanceTitle",
                        "Need Assistance?"
                      )}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {t(
                        "walkinSlot.assistanceDesc",
                        "Staff members are available at the reception desk for help"
                      )}
                    </p>
                    <Button
                    ...
                    >
                      {t("walkinSlot.assistanceButton", "Get Help")}
                    </Button>
                  </div>
                </CardContent>
              </Card>
