...
export default function LabPage() {
  ...
  const existingTokenFromRoute =
    ...
    typeof (location.state as any).token === "string"
      ? ((location.state as any).token as string)
      : undefined;
  ...
  const [activeBooking, setActiveBooking] =
    useState<LabBookingSummary | null>(null);
  ...
  const orderedTests = tests.filter((test) => test.ordered);
  const additionalTests = tests.filter((test) => !test.ordered);
  const selectedAdditionalTests = additionalTests.filter(
    (test) => test.selected
  );

  const orderedTotal = orderedTests.reduce(
    (sum, test) => sum + test.price,
    0
  );
  const additionalTotal = selectedAdditionalTests.reduce(
    (sum, test) => sum + test.price,
    0
  );
  const grandTotal = orderedTotal + additionalTotal;
  ...
  const handlePortalSendOtp = async () => {
    ...
    try {
      ...
      setPortalOtpSessionId(data.otpSessionId || null);
      toast({
        title: t("lab.sendOtp", "OTP Sent"),
        description:
          t("lab.otpSentDesc", "Code sent to") +
  ...
  const handlePortalVerifyOtpAndFetch = async () => {
    ...
    try {
      ...
      // Fetch lab bookings for this patient
      const resp = await fetch(
        `${API_BASE}/api/lab/bookings/by-patient?patientId=${encodeURIComponent(
          patientId
        )}`
      );
      const list: LabBookingSummary[] =
      ...
      const booking = {
        ...list[0],
        existingToken:
          list[0].existingToken || existingTokenFromRoute,
      };

      // Merge tests into local state
      const mergedTests: LabTest[] = [
        ...(booking.orderedTests || []).map((t) => ({
          ...t,
          ordered: true,
        })),
        ...(booking.additionalTests || []).map((t) => ({
          ...t,
          ordered: false,
        })),
      ];
      setTests((prev) =>
        mergedTests.length > 0 ? mergedTests : prev
      );
  ...
  const handleWalkinSendOtp = async () => {
    ...
    try {
      ...
      toast({
        title: t("lab.sendOtp", "OTP Sent"),
        description:
          t("lab.otpSentTo", "We’ve sent an OTP to") +
          ` ${data.normalizedPhone || digits}.`,
      });
    } catch (err: any) {
      setLookupError(
        err?.message ||
          t(
            "lab.walkinOtpSendFailed",
  ...
  const handleStartWalkinFlow = async () => {
    ...
    if (!name || !digits || !walkinYearOfBirth) {
      setLookupError(
        t(
          "lab.walkinDetailsRequired",
          "Please fill name, mobile, year of birth, and verify OTP."
        )
      );
      return;
    }
    ...
    try {
      ...
      toast({
        title: t(
          "lab.walkinRegisteredTitle",
          "Walk-in Registered"
        ),
        description: t(
          "lab.walkinRegisteredDesc",
          "You can now select tests for your lab visit."
        ),
      });
    } catch (err: any) {
      setLookupError(
  ...
  const handleProceedToPayment = async () => {
    if (grandTotal === 0) {
      toast({
        variant: "destructive",
        title: t(
          "lab.noTestsSelectedTitle",
          "No Tests Selected"
        ),
        description: t(
          "lab.noTestsSelectedDesc",
    ...
    if (!bookingSource || !booking) {
      toast({
        variant: "destructive",
        title: t("lab.missingBookingTitle", "Missing Booking"),
        description: t(
          "lab.missingBookingDesc",
          "Please complete the identification step first."
        ),
      });
      return;
    }
    ...
    if (!patientId) {
      toast({
        variant: "destructive",
        title: t("lab.sessionExpiredTitle", "Session Expired"),
        description: t(
          "lab.sessionExpiredDesc",
          "Please go back and verify your details again."
        ),
      });
      return;
    }
    ...
    if (bookingSource === "walkin") {
      try {
          ...
          `${API_BASE}/api/lab/bookings/walkin`,
          {
            ...
            body: JSON.stringify({
              ...
              phone:
                ...
                "",
              tests: [...orderedTests, ...selectedAdditionalTests].map(
                (t) => ({
                  id: t.id,
                  name: t.name,
                  price: t.price,
        ...
        if (!res.ok)
          throw new Error(
            data.detail || `Booking failed (${res.status})`
          );
        appointmentId = data.appointmentId as string;
        // update local booking
        setActiveBooking((prev) =>
        ...
        toast({
          variant: "destructive",
          title: t(
            "lab.createBookingFailedTitle",
            "Failed to create lab booking"
          ),
          description:
            err?.message ||
            t("lab.createBookingFailedDesc", "Please try again."),
        });
        return;
      }
    }
    ...
    if (!appointmentId) {
      toast({
        variant: "destructive",
        title: t(
          "lab.missingAppointmentTitle",
          "Missing Appointment"
        ),
        description: t(
          "lab.missingAppointmentDesc",
          "Could not determine appointment for payment."
        ),
      });
      return;
    }
  ...
  const handleGenerateToken = async () => {
    const booking = activeBooking;
    const totalTests =
      orderedTests.length + selectedAdditionalTests.length;
    ...
    if (!totalTests) {
      toast({
        ...
        description: t(
          "lab.noTestsSelectedTokenDesc",
          "Please select at least one test for the lab visit."
        ),
      });
      return;
    }
    ...
    if (!bookingSource || !booking) {
      toast({
        variant: "destructive",
        title: t(
          "lab.patientTypeNotSelectedTitle",
          "Patient Type Not Selected"
        ),
        description: t(
          "lab.patientTypeNotSelectedDesc",
          "Please choose whether you booked online or are a walk-in patient."
        ),
      });
      return;
    }
    ...
    if (!patientId) {
      toast({
        ...
        description: t(
          "lab.sessionExpiredDesc",
          "Please verify your details again."
        ),
      });
      return;
    }
    ...
    if (bookingSource === "portal" && booking.paid === false) {
      toast({
        variant: "destructive",
        title: t("lab.paymentPendingTitle", "Payment Pending"),
        description: t(
          "lab.paymentPendingDesc",
          "Please complete payment before generating a lab token."
        ),
      });
      return;
    }
    ...
    if (booking.existingToken) {
      toast({
        title: t(
          "lab.tokenAlreadyIssuedTitle",
          "Token Already Issued"
        ),
        description: t(
          "lab.tokenAlreadyIssuedDesc",
          `Your token is ${booking.existingToken}. Please wait for your turn.`
        ),
      });
      return;
    }
    ...
    if (!appointmentId) {
      toast({
        ...
        description: t(
          "lab.missingAppointmentTokenDesc",
          "Could not determine appointment for token generation."
        ),
      });
      return;
    }
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/kiosk/checkin/issue`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (!res.ok)
        throw new Error(
          data?.detail || `Token error (${res.status})`
        );
      ...
      toast({
        title: t(
          "lab.tokenGeneratedTitle",
          "Lab Token Generated"
        ),
        description: t(
          "lab.tokenGeneratedDesc",
          `Your lab token: ${tokenNo}`
        ),
      });
    } catch (err: any) {
      toast({
        variant: "destructive",
        title: t(
          "lab.tokenGenerationFailedTitle",
          "Token Generation Failed"
        ),
        description:
          err?.message ||
          t(
            "lab.tokenGenerationFailedDesc",
  ...
  const handlePrintBarcodes = () => {
    ...
    if (!totalTests) {
      toast({
        ...
        description: t(
          "lab.printLabelsNoTestsDesc",
          "Select tests first so we can print barcode labels."
        ),
      });
      return;
    }
    ...
    toast({
      title: t(
        "lab.printLabelsTitle",
        "Barcode Labels Printing"
      ),
      description: t(
        "lab.printLabelsDesc",
        "Lab barcode labels will be printed for sample collection."
      ),
    });
  };

  const totalTestsForToken =
    orderedTests.length + selectedAdditionalTests.length;
  const canGenerateToken =
    totalTestsForToken > 0 &&
    (bookingSource === "walkin" ||
      activeBooking?.paid !== false);

  return (
    <KioskLayout
      title={
        t("lab.title") ||
        t("lab_services_title", "Laboratory Services")
      }
    >
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-primary mb-4">
            {t("lab.title", "Laboratory Services")}
          </h1>
          <p className="text-lg text-muted-foreground">
            {t(
              "lab.subtitle",
              ...
              <div className="mt-4 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium flex items-center gap-2 mb-1">
                      <Phone className="h-4 w-4 text-muted-foreground" />
                      {t("lab.mobileNumber", "Mobile Number")}
                    </label>
                    <input
                      className="w-full rounded-md border px-3 py-2 text-sm"
                      placeholder="+91 98765 43210"
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <input
                          className="w-full rounded-md border px-3 py-2 text-sm"
                          placeholder={t(
                            "lab.yearOfBirthPlaceholder",
                            "e.g. 1990"
                          )}
                          maxLength={4}
                          value={walkinYearOfBirth}
                          onChange={(e) => {
                            setWalkinYearOfBirth(
                ...
                <div className="space-y-3 mt-4">
                  ...
                  <span className="text-xs text-muted-foreground mt-2 sm:mt-1">
                    {t(
                      "lab.walkinHint",
                      "For walk-in appointment patients who also need lab tests, use the same phone number as your visit. Your existing token will be reused when possible."
                    )}
                  </span>
                </div>
              </div>
            ...
            <Card className="lg:col-span-2 p-6">
              <div className="mb-6">
                <h2 className="text-xl font-semibold mb-2 flex items-center gap-2">
                  <Calendar className="h-5 w-5 text-primary" />
                  {t(
                    "lab.doctorOrderedTitle",
                    "Doctor Ordered Tests"
                  )}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {t(
                    "lab.doctorOrderedSubtitle",
                    "These tests have been prescribed by your doctor (if any)"
                  )}
                </p>
              </div>
              ...
              <div className="mb-4">
                <h3 className="text-lg font-semibold mb-2">
                  {t(
                    "lab.additionalTestsTitle",
                    "Additional Tests (Optional)"
                  )}
                </h3>
                <p className="text-sm text-muted-foreground mb-4">
                  {t(
                    "lab.additionalTestsSubtitle",
                    "Add extra tests to your package"
                  )}
                </p>
              </div>
            ...
            <div className="space-y-6">
              ...
              <Card className="p-6">
                <h3 className="text-lg font-semibold mb-4">
                  {t("lab.billSummaryTitle", "Bill Summary")}
                </h3>
                ...
                <div className="space-y-3">
                  ...
                  {activeBooking?.paid === false &&
                    bookingSource === "portal" && (
                      <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-1 mt-2">
                        {t(
                          "lab.paymentPendingBillNote",
                          "Payment is pending for this online booking. Please complete payment before sample collection."
                        )}
                      </p>
                    )}
                ...
                <div className="space-y-3">
                  {totalTestsForToken > 0 && (
                    ...
                    >
                      ...
                      {activeBooking?.existingToken
                        ? t(
                            "lab.useExistingTokenLabel",
                            "Use Token __TOKEN__"
                          ).replace(
                            "__TOKEN__",
                            String(activeBooking.existingToken)
                          )
                        : t(
                            "lab.generateLabToken",
                            "Generate Lab Token"
                          )}
                    </Button>
                  )}
              ...
              <Card className="p-4 bg-muted/30 border-0">
                ...
                <ul className="text-sm text-muted-foreground space-y-1">
                  ...
                  <li>
                    •{" "}
                    {t(
                      "lab.instructionsList.2",
                      "Reports available in 10 hours"
                    )}
                  </li>
                  <li>
                    •{" "}
                    {t(
                      "lab.instructionsList.3",
                      "Collect reports from lab counter or patient portal"
                    )}
                  </li>
                </ul>
              </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center">
              ...
              <h4 className="font-medium mb-1">
                {t("lab.services.title", "Sample Collection")}
              </h4>
              <p className="text-sm text-muted-foreground">
                {t(
                  "lab.services.location",
            ...
            <div className="text-center">
              ...
              <h4 className="font-medium mb-1">
                {t("lab.services.hoursTitle", "Collection Hours")}
              </h4>
              <p className="text-sm text-muted-foreground">
                {t(
                  "lab.services.hoursText",
                  "7:00 AM – 11:00 AM"
                )}
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <h4 className="font-medium mb-1">
                {t(
                  "lab.services.reportTitle",
                  "Report Delivery"
                )}
              </h4>
              <p className="text-sm text-muted-foreground">
                {t(
                  "lab.services.reportText",
                  "within 10 hours"
                )}
              </p>
            </div>
